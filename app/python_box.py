...
def nms(boxes):
    ...
    _, sort_ids = np.sort(boxes_confs_inv)
    x1 = boxes[:, 0]
    y1 = boxes[:, 1]
    x2 = boxes[:, 2]
    y2 = boxes[:, 3]
    ...
    for index in range(no_of_valid_elems):
        ...
        if float(boxes[i, 4]) > conf_thresh:
            xx1 = np.max(x1[i], x1[sort_ids[new_ind:]])
            yy1 = np.max(y1[i], y1[sort_ids[new_ind:]])
            xx2 = np.min(x2[i], x2[sort_ids[new_ind:]])
            yy2 = np.min(y2[i], y2[sort_ids[new_ind:]])
            w = np.max(np.zeros(1), xx2 - xx1 + 1)
            h = np.max(np.zeros(1), yy2 - yy1 + 1)
...
def get_region_boxes(output):
    ...
    anchors = np.empty(num_anchors * 2)
    ...
    grid_x = np.linspace(0, w-1, w).repeat(h,
                                           1).repeat(batch*num_anchors, 1, 1).view(batch*num_anchors*h*w)
    grid_y = np.linspace(0, h-1, h).repeat(w,
                                           1).t().repeat(batch*num_anchors, 1, 1).view(batch*num_anchors*h*w)
    xs = np.sigmoid(output[0]) + grid_x
    ys = np.sigmoid(output[1]) + grid_y
    ...
    anchor_h = anchor_h.repeat(batch, 1).repeat(
        1, 1, h*w).view(batch*num_anchors*h*w)
    ws = np.exp(output[2]) * anchor_w.float()
    hs = np.exp(output[3]) * anchor_h.float()

    det_confs = np.sigmoid(output[4])

    cls_confs = np.softmax(output[5: 5+num_classes].transpose(0, 1), dim=1)
    cls_max_confs, cls_max_ids = np.max(cls_confs, 1)
    cls_max_confs = cls_max_confs.view(-1)
    cls_max_ids = cls_max_ids.view(-1)

    sz_hw = h * w
    boxes = np.zeros(batch, h * w * num_anchors, 7)
    ...
    if no_selected_elems > 0:
        boxes[:, 0:no_selected_elems] = np.stack(
